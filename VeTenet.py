...
class VeTenet:
    ...
    def vetra(self,deltaThreshold,WCCsizeCutoff,clusternumber):
        ...
        for k in range(len(embeddingGraphShortestpath)):
            ...
            plt.scatter(embedding[pathway,0],embedding[pathway,1],  marker='o', c = scaled_Lambda,s=100)
            plt.axis("off")
            plt.title('trajectory_'+str(k+1))
            plt.savefig(outfolder+'/'+file_name+'trajectory_'+str(k+1)+'.pdf', dpi=600)
